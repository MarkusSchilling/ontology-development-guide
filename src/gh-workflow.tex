...
An example for this workflow and instructions on how to create your own can be found in our example repository.\footnote{\url{https://github.com/scientific-ontology-network/example-ontology-repository}}
...
\begin{itemize}
    ...
    \item add issue number to PR, use "closes \#[IssueNumber]" to make {\github} close the issue once the PR is merged
...
Betty has a change request, because Dillan missed to add "microscope" to the list of added terms in the changelog and marks this in the PR. Eric is content with the implementation and approves the PR, after Dillan added the note the changelog.

After the approval and after the automated checks (CI) in {\github} were successful, Dillan merges the PR to the development branch. The term "microscope" has been added to the ontology.
\end{example}
