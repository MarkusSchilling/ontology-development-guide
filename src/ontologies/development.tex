...
\begin{example}
Alice and other experts brain-storm important concepts for the domain of car manufacturing. Afterwards, this list is filtered with respect to the scope defined earlier, compiled into a list of todos for future ontology development and documented.
\end{example}
...
\begin{example}
The list of important concepts compiled in the previous step is then checked against several other relevant ontologies in order to identify terms that are already defined elsewhere. Using the terminology service, Alice notices that multiple important concepts are already defined elsewhere. Using the terminology service, Alice notices that multiple important concepts are already defined in other ontologies, e.g. \href{https://pi.pauwel.be/voc/buildingelement#MechanicalFastener-BOLT}{bolt} and \href{http://purl.obolibrary.org/obo/NCIT_C49869}{chassis}. Upon further inspection, however, she realises that the definition of bolt provided in this ontology is largely focused on its use in buildings. In particular its superclass \textit{`MechanicalFastener'}, defined as \textit{`A fastener connecting building elements mechanically.[...]' does not fit the intended use for her target domain. Therefore, she opts to not reuse this existing definition.} 
\end{example}

\begin{example}
    Although Alice already limited the scope of her ontology to primarly include terms describing parts of cars and processes used to assemble these parts, she notices that there are still many relevant terms. To lower the difficulty of maintenance, she opts for a modular approach. 

    She defines a shared core ontology that imports BFO and defines the most general terms that are relevant for all modules e.g. \texttt{Car}, \texttt{Motor}, \texttt{Combustion Engine} and \texttt{SafetyMeasure}. She then defines a module defining the various parts and their assembling processes like \texttt{Combustion Engine Assembly Process}, as well as one module containing all the terms covering safety-related aspects like \texttt{Airbag} and \texttt{Crash Test}. Finally, in order for anyone to use the combination of all terms, she defines the main ontology that imports these two modules.

    With this approach axioms between the modules can only be established for the most general terms in the shared core ontology, meaning there are no dependencies between the modules.
\end{example}
...
To get along the path of developing good definitions, it is advisable to follow some best practices.
...
\begin{example}
    Alice decides to define the term \texttt{Motor} in the ontology and intuitively comes up with the definition "A motor is a machine that makes things move." This definition however is not precise by using vague terms like "machine" and "makes things move" and would even classify objects like a windmill as a motor.
    
    After further refinement she comes up with the following definition: "A motor is an energy converting component that converts other forms of energy into kinetic energy." This definition is more precise and allows an easier definition of the axioms e.g. \texttt{'has energy output' some 'kinetic energy'}.
\end{example}
...
\subsubsection{Separate Terminological and Factual Knowledge}
